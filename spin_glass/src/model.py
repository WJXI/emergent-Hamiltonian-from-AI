...
class PairformerBlock(nn.Module):
    ...
    def forward(self, s, z, mask=None):
        ...
        # Update single representation s through pair-biased attention.
        ...
        attn_logits = attn_logits + self.z_to_bias(z).permute(0, 3, 1, 2)
...
# O(3)-equivariant scalar aggregation followed by tangent projection.
class MiniAF3ScoreModel(nn.Module):
    def __init__(self, c_s=64, c_z=32, num_blocks=4):
        ...
        # The network predicts scalar pair coefficients, not Cartesian vectors.
    ...
    def forward(self, seq, spins, t, mask):
        ...
        # Read the dense scalar matrix W_ij.
        W = self.weight_head(z).squeeze(-1) # (B, N, N)
        
        # Reciprocal exchange coefficients are symmetric under i <-> j.
        W = (W + W.transpose(1, 2)) / 2.0
        
        # There is no self-interaction, so the diagonal is zero.
        diag_mask = 1.0 - torch.eye(N, device=device).unsqueeze(0) # (1, N, N)
        W = W * diag_mask
        # Remove padded pairs.
        W = W * mask.float().unsqueeze(1) * mask.float().unsqueeze(2)
        
        # Equivariant aggregation: sum_j W_ij S_j.
        raw_score = torch.bmm(W, spins) # (B, N, 3)
        
        # Project the ambient vector onto T_{S_i} S^2.
...
def sample_spherical_noise_and_target(S_0, t_batch):
    """
    Perturb spins with the spherical exponential map and return Log/t.

    The logarithmic-map target is the leading small-time conditional score of
    the heat kernel used by the Riemannian DSM objective.
    """
    # Tangent Gaussian followed by the exponential map.
    v = torch.randn_like(S_0)
    v_tan = v - torch.sum(v * S_0, dim=-1, keepdim=True) * S_0
    v_norm = torch.norm(v_tan, dim=-1, keepdim=True) + 1e-8
    u = v_tan / v_norm
    theta = v_norm * torch.sqrt(t_batch)
    S_t = S_0 * torch.cos(theta) + u * torch.sin(theta)
    
    # Conditional target (1/t) Log_{S_t}(S_0), tangent at S_t.
